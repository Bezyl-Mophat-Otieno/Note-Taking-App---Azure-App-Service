...
export default function GoalTracker() {
  ...
  // Azure App Service Easy Auth integration
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (authData.length > 0) {
            ...
            setUser({
              name: userInfo.user_claims?.find((c: any) => c.typ === "name")?.val || userInfo.user_id,
              email:
                userInfo.user_claims?.find(
                  (c: any) => c.typ === "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress",
                )?.val || "",
              picture: userInfo.user_claims?.find((c: any) => c.typ === "picture")?.val,
            })
          }
        }
      } catch (error) {
